...
parser.add_argument("--mode", choices=["label", "color"], default="label", help="Display mode: 'label' shows [Speaker], 'color' relies on text color.")
...
SPEAKER_COLORS = {
    "[Speaker 0]": (240, 240, 240),
    "[Speaker 1]": (255, 223, 130),
    "[Speaker 2]": (163, 255, 177),
    "[Speaker 3]": (177, 163, 255),
    "[Speaker 4]": (255, 163, 177)
}
...
async def pygame_loop():
    ...
    while True:
        ...
        for item in all_items:
            ...
            display_name = current_speaker.replace("SPEAKER_", "Speaker ").replace("speaker", "Speaker ")
        ...
        if state["sound"] and (current_time - state["sound_timestamp"] > SOUND_DISPLAY_DURATION):
            state["sound"] = ""
...
async def main():
    uri = "wss://wolf-arms-ipaq-farm.trycloudflare.com/ws"
    try:
