...
parser.add_argument("--phrase_timeout", default=0.6, type=float, help="Silence gap (sec) to trigger final transcription.")
parser.add_argument("--max_duration", default=3.0, type=float, help="Max duration before forcing a final result.")
parser.add_argument("--vad_threshold", default=0.4, type=float, help="VAD sensitivity (lower = more sensitive).")
...
speech_model = WhisperModel("deepdml/faster-whisper-large-v3-turbo-ct2", device=device, compute_type="float16")
...
class WebSocketAudioSource(AudioSource):
    ...
    def read(self): pass
    def close(self): self.stream.on_completed()
...
speaker_timeline = collections.deque(maxlen=50) #store recent speaker labels with timestamps
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    pre_roll = collections.deque(maxlen=10) #keeps audio just before speech starts to avoid cutting off beginning of phrases
    ...
    async def process_audio_task(audio_data, speaker_at_capture, is_final=True):
        ...
        async with gpu_lock:
            ...
            try:
                result = await loop.run_in_executor(whisper_executor, get_speech, audio_data, is_final)
                text = result['text'].strip()
                if text:
                    msg_type = "final" if is_final else "partial"
                    await websocket.send_json({"type": msg_type, "text": text, "speaker": speaker_at_capture})
    ...
    try:
        while True:
            ...
            loop.run_in_executor(diart_executor, audio_source.push_audio, audio_chunk.copy())
            ...
            if chunk_counter % 8 == 0 and len(yamnet_buffer) == 16384:
                async def ps(buf):
                    try:
                        s, sc = await loop.run_in_executor(sound_executor, get_sounds, buf)
            ...
            if speech_prob > args.vad_threshold:
                ...
                if len(voiced_buffer) % 4 == 0:
                    ...
                    asyncio.create_task(process_audio_task(np.concatenate(voiced_buffer), speaker_snapshot, is_final=False))
                
                #Force final transcriptions if buffer gets too long
                if (len(voiced_buffer) * CHUNK_SIZE) / SAMPLE_RATE >= args.max_duration:
                    speaker_snapshot = get_speaker_at(utterance_start_time)
                    asyncio.create_task(process_audio_task(np.concatenate(voiced_buffer), speaker_snapshot, is_final=True))
                    voiced_buffer = []
                    is_speaking = False
                    pre_roll.clear() 
            else:
